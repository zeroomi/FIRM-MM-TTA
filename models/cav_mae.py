...
class CAVMAEFT(nn.Module):
    ...
    def forward_eval_with_features(self, a, v):
        ...
        attn_dict = {'audio_only': None, 'video_only': None, 'full': None}
        ca = cv = None  # only the 'full' branch may produce
        
        if video_only.any():
            x_v, attn_v, f_v = self.forward_video_only(v[video_only]) 
            logits = torch.index_put(logits, (video_only,), x_v)
            feat[video_only]   = f_v
            attn_dict['video_only'] = attn_v

        if audio_only.any():
            x_a, attn_a, f_a = self.forward_audio_only(a[audio_only])
            logits = torch.index_put(logits, (audio_only,), x_a)
            feat[audio_only]   = f_a
            attn_dict['audio_only'] = attn_a

        if full.any():
            x_f, attn_f, ca, cv, f_f = self.forward_full(a[full], v[full])
            logits = torch.index_put(logits, (full,), x_f)
            feat[full]   = f_f
            attn_dict['full'] = attn_f
