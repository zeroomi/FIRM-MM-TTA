...
class AudiosetDataset(Dataset):
    ...
    def pro_data(self, data_json):
        for i in range(len(data_json)):
            if 'missing_a' not in data_json[i]:
                data_json[i] = [data_json[i]['wav'], data_json[i]['labels'], data_json[i]['video_id'], data_json[i]['video_path']]
            else:
                data_json[i] = [data_json[i]['wav'], data_json[i]['labels'], data_json[i]['video_id'], data_json[i]['video_path'], data_json[i]['missing_a'], data_json[i]['missing_v']]
    ...
    def decode_data(self, np_data):
        ...
        if len(np_data) > 4:
            datum['missing_a'] = np_data[4]
            datum['missing_v'] = np_data[5]
        return datum
    ...
    def __getitem__(self, index):
        ...
        if 'missing_a' in datum:
            if datum['missing_a'] == 'True':
                fbank = torch.zeros_like(fbank)
                
        if 'missing_v' in datum:
            if datum['missing_v'] == 'True':
